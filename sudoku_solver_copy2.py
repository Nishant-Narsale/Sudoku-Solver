board = [
    [0,0,0,0,0,0,8,1,7],
    [0,2,0,9,0,0,0,3,0],    #backtrack calling for this file is temporary removed because it is running inifinite loop
    [8,0,0,7,0,0,9,0,0],
    [5,0,0,0,8,0,0,0,0],
    [0,0,3,1,0,2,5,0,0],
    [0,0,0,0,4,0,0,0,6],
                ...
                for k in range(len(board)):
                    ...
                    if not in_box:
                        ...
                        if start_from == 1:
                            ...
                            assigned_numbers[len(assigned_numbers)-backtrack_counter][2] = i
                        
                        backtrack_counter = 0
                        break
                        
            if (i == 9 and board[empty_row][empty_col] == 0) or (i==9 and backtrack_counter > 1 and board[empty_row][empty_col] == start_from-1):
                board[empty_row][empty_col] = 0
                backtrack_counter += 1
                
                
def backtrack():
    # now applying backtracking algorithm 
    print('backtracking')
            
    """now as we can see i value has crossed 9 but still empty value has not changed,
    so, we have to go back to before assigned value and change it to next suitable value."""
    
    row , col , z = assigned_numbers[len(assigned_numbers)-backtrack_counter]
    print(row,col,z)
    
    """for changing it to next suitable value instead of previously choosed we have to check for
    one value more than firstly assigned value """
    
    valid_number(board, row, col, start_from=z+1)
